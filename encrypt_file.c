/*
 * Compile: gcc -o encrypt_file aes.c encrypt_file.c
 * Usage: encrypt_file src_file dst_file -k key
 */
...
int writeContentToFile(unsigned char *content, unsigned char *filePath) {
  if (!content || !filePath) {
    printf("content or filePath null\n");
    return 0;
  }
...
int main(int argc, char *argv[]) {
  ...
  // check command args
  if (argc != 3 && argc != 5) {
    printf("encrypt_dir src_file dst_file -k key\n");
    return -1;
  }
  ...
  unsigned char *encryptContent = aes_encrypt_cbc_str(key, content);
  writeContentToFile(encryptContent, dstFile);
